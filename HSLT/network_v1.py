...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, input_dim, heads, dropout=0.4, D_h=64, k=16):
        ...
        self.msa = nn.MultiheadAttention(embed_dim=D_h, num_heads=k, dropout=dropout, batch_first=True)
        self.fc2 = nn.Linear(D_h, input_dim)
    ...
    def forward(self, x):
        ...
        attn_output = F.relu(self.fc2(attn_output))
        y = x + attn_output
...
class TransformerEncoder(nn.Module):
    def __init__(self, input_dim, heads, num_layers=2, dropout=0.4, D_h=64, k=16):
        ...
        self.layers = nn.ModuleList([TransformerEncoderLayer(input_dim=input_dim, heads=k, dropout=dropout) for _ in range(num_layers)])
